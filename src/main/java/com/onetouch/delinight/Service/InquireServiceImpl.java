...
public class InquireServiceImpl implements InquireService {
    ...
    public InquireDTO register(InquireDTO inquireDTO, Long roomId, Long usersId) {
        ...
        //Inquire에 연결 정보 설정
        inquireEntity.setCheckInEntity(checkInEntity); //체크인 기록
        inquireEntity.setHotelEntity(hotelEntity); //호텔 정보
        inquireEntity.setUsersEntity(usersEntity); //사용자 정보

        //저장
        inquireEntity = inquireRepository.save(inquireEntity);

        //결과를 DTO로 변환해서 반환
        return modelMapper.map(inquireEntity, InquireDTO.class);
